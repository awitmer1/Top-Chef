...
function searchCuisines(allDishes) {
    let userCuisineSearch = prompt("What type of cuisine would you like to search?");
    alert(`Searching for ${userCuisineSearch} dishes...`)
    let foundCuisine = allDishes.filter(function(el){
        if (el.cuisine.includes(userCuisineSearch)) {
            return true;
        } else {
            return false;
        }
...
function searchIngredients(allDishes) {
    let userIngredientSearch = prompt("Which ingredient would you like to search?")
    alert(`Searching for ${userIngredientSearch} dishes...`)
    let foundIngredient = allDishes.filter(function(el){
        if (el.ingredients.includes(userIngredientSearch)) {
            return true;
        } else {
            return false;
        }
...
function generateMarketingMessage(dishOfTheDay, emailMessage) {
    ...
    let message = emailMessage(dishOfTheDay)
    console.log(message)
...
function runApp(allDishes, specialDish) {
    ...
    switch (userChoice) {
        ...
        case "7":
            ...
            // You will need to provide today's dish and the appropriate callback function as arguments!
            let dishOfDayEmail = generateMarketingMessage(todaysSpecialDish,emailMessage)
